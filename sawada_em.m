function [model] = sawada_em(X,K,verb,maxiter,tol)
%%%%%%%%%%%%%%%%%% EM-algorithm for Sawada's BSS model %%%%%%%%%%%%%%%%%%%%
% The model is:
%     p(x|z=k) = N(x;(a_k'*x)*a_k,sigma2_k*eye(D))
%     p(z=k) = pi_k
...
converged = 0;
niter = 0;
if(verb>0);fprintf(1,'Sawada algorithm running... iter');end;
while(~converged)
    niter = niter + 1;
    if(verb>0);fprintf(1,' %d',niter);end;    
    % Expectation
    ...
    % Check convergence
    if norm(old_R(:)-R(:))/norm(old_R) < tol || niter>=maxiter
        converged = true;
    end
    old_R = R;
end
% fill field alpha for consistency
...
function R = expectation(X,model)
    ...
    for k=1:K
        a_k = model.A(:,k);
        % Subtract projection of X on A_k
        mu = X - bsxfun(@times,a_k'*X,a_k); % D*T
        % Covariance matrix
        Sigma_k = model.sigma2(k)*eye(D);
        % Ensures positive-definiteness :
        logR(k,:) = log(model.w(k)) + loggausspdf(mu,Sigma_k); % 1*N
    end
...
function model = maximization(X,R)
    ...
    for k=1:K       
        ...
        [v_k,~,~] = svd(Gamma_k); % D*1, double 
        v_k = v_k(:,1);
        % Update steering vector
        model.A(:,k) = v_k./norm(v_k);
        
        % Subtract projection of X on A_k
        mu = X - bsxfun(@times,model.A(:,k)'*X,model.A(:,k)); % D*T
        
        % Update noise variance
        model.sigma2(k) = sum(sum(abs(mu).^2,1).*R(k,:))./(D*sumRk);
    end
